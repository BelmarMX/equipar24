import{n}from"./common-Bm_rDy6i.js";import{A as p}from"./alerts-ClLvi9ZR.js";import"./sweetalert2.esm.all-Br4OTrmy.js";$(document).ready(function(){const l=e=>{let a=0,t=0;$("[data-product_id]").each((o,s)=>{let d=$(s).data("product_id"),i=$('[name="quotation.in_stock['+d+']"]').val()==="1",r=$('[name="quotation.is_deleted['+d+']"]').val()==="1";if(!i||r)return;let c=parseFloat($('[name="quotation.quantity['+d+']"]').val()),u=parseFloat($('[name="quotation.total['+d+']"]').val()),m=parseFloat($('[name="quotation.original['+d+']"]').val()),h=c*u;a+=h,t+=m,$('[data-update-amount="'+d+'"]').text(`$${n(h)}`)}),console.log(t),$("[data-original-price]").text(`$${n(t)}`),$("[data-gran-total]").text(`$${n(a)}`)};$(document).on("change",'[data-table="quantity"]',function(e){l()}),$(document).on("click","[data-no-stock]",function(e){e.preventDefault();const a=$(this).attr("data-no-stock"),t=$(this);p.confirm(`El producto ${a} será marcado sin existencias y no podrá ser restaurado en la cotización.`,o=>{t.remove(),$('[data-product_id="'+a+'"]').addClass("bg-red-50"),$('[data-if-not-stock="'+a+'"]').html(""),$('[data-delete="'+a+'"]').remove(),$('[data-restore="'+a+'"]').remove(),$('[name="quotation.in_stock['+a+']"]').val(0),$('[name="quotation.is_deleted['+a+']"]').val(1),l()})}),$(document).on("click","[data-delete]",function(e){e.preventDefault();const a=$(this).attr("data-delete"),t=$(this);if($(document).find("tr[data-product_id]:not(.bg-red-50):not(.bg-red-100)").length<=1)return p.warning("La cotización debe tener por lo menos un producto activo","¡Cuidado!");p.confirm(`El producto ${a} será eliminado de la cotización.`,s=>{t.addClass("hidden"),$('[data-restore="'+a+'"]').removeClass("hidden"),$('[data-product_id="'+a+'"]').addClass("bg-red-100"),$('[name="quotation.quantity['+a+']"]').attr("readonly","readonly"),$('[name="quotation.is_deleted['+a+']"]').val(1),l()})}),$(document).on("click","[data-restore]",function(e){e.preventDefault();const a=$(this).attr("data-restore"),t=$(this);p.confirm(`Se restaurará el producto ${a} en la cotización.`,o=>{t.addClass("hidden"),$('[data-delete="'+a+'"]').removeClass("hidden"),$('[data-product_id="'+a+'"]').removeClass("bg-red-100"),$('[name="quotation.quantity['+a+']"]').removeAttr("readonly"),$('[name="quotation.is_deleted['+a+']"]').val(0),l()})}),$("#send_whatsapp").on("click",function(e){let a=$('[data-field="contact_phone"] > input').val(),t=$('[data-field="contact_name"] > input').val(),o="";$(".quotation > tr:not(.ignore)").each((d,i)=>{let r=$('[name="quotation.in_stock['+$(i).data("product_id")+']"]').val()==="1",c=$('[name="quotation.is_deleted['+$(i).data("product_id")+']"').val()==="1",u=$(i).find('[data-table="quantity"]').val();(!r||c)&&(u="0");let m=$(i).find('[data-table="title"]').text();o+=`${u} x ${m}${!r||c?" (No disponible)":""}

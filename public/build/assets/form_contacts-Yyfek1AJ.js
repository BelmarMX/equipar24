import{e as f,n}from"./common-CQF4xCex.js";import{A as s}from"./alerts-ClLvi9ZR.js";import"./sweetalert2.esm.all-Br4OTrmy.js";$(document).ready(function(){const l=e=>{let a=0,t=0;$("[data-product_id]").each((o,r)=>{let d=$(r).data("product_id"),i=$('[name="quotation.in_stock['+d+']"]').val()==="1",c=$('[name="quotation.is_deleted['+d+']"]').val()==="1";if(!i||c)return;let u=parseFloat($('[name="quotation.quantity['+d+']"]').val()),p=parseFloat($('[name="quotation.total['+d+']"]').val()),m=parseFloat($('[name="quotation.original['+d+']"]').val()),h=u*p;a+=h,t+=m,$('[data-update-amount="'+d+'"]').text(`$${n(h)}`)}),console.log(t),$("[data-original-price]").text(`$${n(t)}`),$("[data-gran-total]").text(`$${n(a)}`)};$(document).on("change",'[data-table="quantity"]',function(e){l()}),$(document).on("click","[data-no-stock]",function(e){e.preventDefault();const a=$(this).attr("data-no-stock"),t=$(this);if($(document).find("tr[data-product_id]:not(.bg-red-50):not(.bg-red-100)").length<=1)return s.warning("La cotización debe tener por lo menos un producto activo","¡Cuidado!");s.confirm(`El producto ${a} será marcado sin existencias y no podrá ser restaurado en la cotización.`,r=>{t.remove(),$('[data-product_id="'+a+'"]').addClass("bg-red-50"),$('[data-if-not-stock="'+a+'"]').html(""),$('[data-delete="'+a+'"]').remove(),$('[data-restore="'+a+'"]').remove(),$('[name="quotation.in_stock['+a+']"]').val(0),$('[name="quotation.is_deleted['+a+']"]').val(1),l()})}),$(document).on("click","[data-delete]",function(e){e.preventDefault();const a=$(this).attr("data-delete"),t=$(this);if($(document).find("tr[data-product_id]:not(.bg-red-50):not(.bg-red-100)").length<=1)return s.warning("La cotización debe tener por lo menos un producto activo","¡Cuidado!");s.confirm(`El producto ${a} será eliminado de la cotización.`,r=>{t.addClass("hidden"),$('[data-restore="'+a+'"]').removeClass("hidden"),$('[data-product_id="'+a+'"]').addClass("bg-red-100"),$('[name="quotation.quantity['+a+']"]').attr("readonly","readonly"),$('[name="quotation.is_deleted['+a+']"]').val(1),l()})}),$(document).on("click","[data-restore]",function(e){e.preventDefault();const a=$(this).attr("data-restore"),t=$(this);s.confirm(`Se restaurará el producto ${a} en la cotización.`,o=>{t.addClass("hidden"),$('[data-delete="'+a+'"]').removeClass("hidden"),$('[data-product_id="'+a+'"]').removeClass("bg-red-100"),$('[name="quotation.quantity['+a+']"]').removeAttr("readonly"),$('[name="quotation.is_deleted['+a+']"]').val(0),l()})}),$("#send_whatsapp").on("click",function(e){let a=$('[data-field="contact_phone"] > input').val(),t=$('[data-field="contact_name"] > input').val(),o="";$(".quotation > tr:not(.ignore)").each((d,i)=>{let c=$('[name="quotation.in_stock['+$(i).data("product_id")+']"]').val()==="1",u=$('[name="quotation.is_deleted['+$(i).data("product_id")+']"').val()==="1",p=$(i).find('[data-table="quantity"]').val();(!c||u)&&(p="0");let m=$(i).find('[data-table="title"]').text();o+=`${p} x ${m}${!c||u?" (No disponible)":""}
`}),o=encodeURI(o.replaceAll("&","").replaceAll("%","").replaceAll("?",""));let r=`https://api.whatsapp.com/send?phone=521${a}&text=Estimado/a%20${t}%0A%0A¡Gracias%20por%20contactar%20a%20Equi-par%20y%20por%20tu%20interés%20en%20nuestros%20productos!%20Antes%20de%20enviarte%20la%20cotización%20final,%20queremos%20asegurarnos%20de%20que%20la%20información%20proporcionada%20sea%20correcta%20y%20confirmar%20algunos%20detalles%20contigo.%0A%0AAdjunto%20encontrarás%20un%20resumen%20de%20los%20productos%20que%20has%20solicitado.%20Además,%20queremos%20informarte%20que%20algunos%20de%20ellos%20podrían%20no%20estar%20disponibles%20en%20este%20momento.%20Sin%20embargo,%20contamos%20con%20opciones%20similares%20que%20cumplen%20con%20las%20mismas%20características%20y%20que%20podrían%20interesarte.%0A%0A${o}%0APor%20favor,%20revisa%20la%20información%20y%20confírmanos%20si%20todo%20es%20correcto%20o%20si%20deseas%20considerar%20las%20alternativas%20que%20podemos%20ofrecerte.%20Estaremos%20encantados%20de%20ajustarnos%20a%20tus%20necesidades.%0A%0AQuedamos%20atentos%20a%20tu%20respuesta.`;window.open(r,"_blank")}),$(document).on("keydown","#search_title",function(e){e.key==="Enter"&&(e.preventDefault(),$("[data-search-product]").click())}),$(document).on("click","[data-search-product]",function(e){e.preventDefault(),$(this).find(".working").removeClass("hidden"),$(this).find(".static").addClass("hidden"),$("#results").html('<option selected="" disabled>Selecciona uno de la lista</option>'),axios.post("/productos/autocomplete",{query:$("#search_title").val()}).then(({data:a})=>{a||($('[data-show="results_not_found"]').removeClass("hidden"),$('[data-show="results_found"]').addClass("hidden")),$('[data-show="results_not_found"]').addClass("hidden"),$('[data-show="results_found"]').removeClass("hidden"),a.sort((t,o)=>t.title.localeCompare(o.title)),a.forEach(t=>{console.log(t.promotion),$("#results").append(`<option value="${t.id}"

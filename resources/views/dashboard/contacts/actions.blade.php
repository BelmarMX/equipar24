@if( !empty($actions['restore']) && $actions['restore']['enabled'] )
    <a class="dt-custom-action restore" href="{{ route($actions['restore']['route'], $record -> id) }}" data-confirm-redirect='¿Quieres restaurar este formulario?' data-tooltip="Restaurar"><i class="fa-solid fa-trash-arrow-up"></i></a>
@else
